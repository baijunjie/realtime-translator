...
- 온디맨드 모델 다운로드 — '녹음 시작'을 누를 때(또는 설정에서 아직 다운로드하지 않은 모델을 선택할 때) 확인 대화상자(모델 이름과 크기 표시)가 뜨고, 확인하면 바이트 진행률로 한 번에 다운로드한 뒤 완료되면 자동으로 이어집니다. 이미 다운로드한 모델은 앱 실행 시 미리 로드되어 '녹음 시작'을 누르면 즉시 녹음이 시작됩니다
- CPU만으로 실시간 동작(Apple Silicon 실측 RTF ≈ 0.03), GPU 불필요

## 사용법

1. **첫 실행** — 온보딩 화면에서 모국어를 선택합니다.
2. **녹음 시작**을 클릭 — 선택한 인식 모델이 아직 다운로드되지 않았다면 확인 대화상자(모델 이름과 크기 표시)가 먼저 뜹니다. 확인 후 완료되면 자동으로 녹음이 시작되고, 말하면 자막이 실시간으로 표시됩니다.
...
macOS / 웹에서는 인식 모델과 로컬 번역 모델 모두 온디맨드로 다운로드됩니다: 처음 '녹음 시작'을 누를 때, 또는 설정에서 아직 다운로드하지 않은 모델을 선택할 때 확인 대화상자가 뜬 뒤 다운로드가 진행됩니다.
